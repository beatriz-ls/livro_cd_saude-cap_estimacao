...
```{r}
# Le o arquivo RDS com os dados da COVID
dados <- readr::read_rds("dados/dados_covid[LIMPO].rds")
# Remove as linhas onde a variavel 'saturacao' esta ausente (NA)
dados <- dplyr::filter(dados, !is.na(saturacao))
# Sorteia um valor aleatorio da coluna 'saturacao'
...
```{r}
# Repete 10 vezes e sorteia um valor aleatorio da coluna
...
É evidente que não é possível prever o resultado de um experimento aleatório, mas uma ferramenta útil para descrever seu comportamento é identificar os possíveis "desfechos". Retomando o exemplo anterior, essa informação pode ser obtida usando a funcao `unique()`, que extrai valores exclusivos do vetor, removendo as ocorrências duplicadas. No codigo abaixo, o vetor analisado armazena as ocorrências de baixa saturacao de oxigenio:

```{r}
# Retorna os valores presentes no vetor
...
Em muitos casos, o espaço amostral de um experimento não será numérico. Por exemplo, retirando três indivíduos da população e observando a sua saturação de oxigênio, obtemos $\Omega=\{(x_1 x_2 x_3)|x_i=\text{sim ou não }, i=1,2,3\}$. Porém, muitas vezes é útil que os resultados dessa variável sejam registrados como números, o que possibilita a realização de algumas mensurações. Neste exemplo, em vez de registrar a categoria da saturação de cada um dos três indivíduos, podemos contar o número de indivíduos cuja resposta é "sim", atribuindo o valor 1 àqueles que apresentam saturação de oxigênio baixa e o valor 0 aos que não apresentam. Isso se enquadra como uma **variável aleatória**, ou seja, uma função $X$ que associa a cada elemento $\omega \in \Omega$ um número real $X(\omega)$.
...
Uma variável é considerada discreta se $R_x$ for um conjunto finito ou infinito enumerável. O número de indivíduos com saturação baixa em uma amostra de tamanho 3, por exemplo, é uma variável aleatória discreta, pois podemos listar todos os seus possíveis valores.
...
```{r}
# Calcula a proporcao de registros da variante "gama" no vetor dados$variante
...
```{r}
# Valores da variavel aleatoria discreta Y
y <- c(0, 1, 2)
# Probabilidades associadas a cada valor de Y
py <- c(0.2353917, 0.4995602, 0.2650481)
# Data.frame dos valores da variavel Y e as probabilidades equivalentes
df <- data.frame(y, py)
df
```

```{r, echo=FALSE}
# Cria graficos de funcao de massa de probabilidade
plot(y, py, type='h', lwd=4, xlab="Valores de Y", ylab="P(Y = y)", main="Função Massa de Probabilidade")
...
F(y) =
    ...
    1 & se \ y \geq 2.
    \end{cases}
$$ Podemos calcular essa função no R:

```{r}
# Carrega o pacote dplyr
library(dplyr)
# Cria uma nova variavel Fy no data frame df, que representa a distribuicao acumulada dos pesos (py)
df <- df |>
...
E representá-la graficamente:

```{r, echo =F}
# Gera uma amostra aleatoria da variavel y com 100.000 observacoes
# A amostragem e feita com reposicao (replace = TRUE) e respeita as probabilidades (pesos) indicadas por py
simulacao <- sample(df$y, size = 100000, prob = df$py, replace = TRUE)

```

Outra medida relevante quando o assunto é variáveis aleatórias é a **esperança**, definida como o valor esperado da variável aleatória, ou seja, o resultado esperado dessa variável aleatória em média a longo prazo. $$
...
```{r}
# Calcula a media ponderada da variavel y, utilizando os pesos py
...
```{r}
# Calcula a variancia ponderada da variavel y no data frame df, usando pesos py
...
Algumas variáveis aleatórias assumem apenas valores inteiros e podem ser representadas por modelos de probabilidade discretos já consolidados na teoria das probabilidades. Esses modelos descrevem o comportamento dessas variáveis e permitem calcular a probabilidade de ocorrência de determinados valores. A seguir, apresentam-se os principais modelos discretos utilizados.

###### Modelo Bernoulli

O modelo de Bernoulli é utilizado para descrever situações em que há apenas duas possibilidades de resultado em um experimento aleatório: a ocorrência ou não ocorrência de um determinado evento, que chamamos de "sucesso". Assim, seu espaço amostral apresenta apenas dois elementos: o valor 1 representando a ocorrência do "sucesso" e o valor 0 indicando o "fracasso", ou seja, a não ocorrência do sucesso. Para cada experimento alestório, definimos qual será o evento denominado de "sucesso". Por exemplo, pensando na ocorrência ou não de cardiopatia em uma gestante ou puérpera diagnosticada com COVID-19 de março de 2020 a dezembro de 2021, podemos definir a presença de cardiopatia como o evento que chamamos de "sucesso" e a não presença como o "fracasso", usando o modelo Bernoulli para compreeder o comportamento dessa variável.

Desse modo, considerando uma probabilidade $p$ de sucesso, a variável aleatória $X$ apresenta a seguinte função massa de probabilidade
...
ggplot(data=dados, aes(x=idade_anos)) +  # Curva de densidade com preenchimento azul claro e contorno preto
  geom_density(fill="lightblue", color = "black") + # Rotulos dos eixos
...
```{r, echo=F}
# Calcula a densidade da variavel idade_anos
densidade <- density(dados$idade_anos)

# Cria um data frame com os valores da densidade para uso na area preenchida do grafico
area_colorida <- data.frame(x = densidade$x, y = densidade$y)

# Filtra os valores da densidade para a faixa de idade entre 0 e 20 anos
area_colorida <- subset(area_colorida, x >= 0 & x <= 20)

# Cria o grafico de densidade
ggplot(dados, aes(x = idade_anos)) +
  geom_density(color = "black") +
  geom_ribbon(data = area_colorida,
              aes(x = x, ymin = 0, ymax = y),
...
Enquanto os modelos discretos são utilizados para variáveis que assumem valores inteiros e bem definidos, os modelos de probabilidade contínuos são aplicados quando a variável aleatória pode assumir qualquer valor dentro de um intervalo real. Nesses casos, em vez de calcular a probabilidade de valores isolados, interessa-nos determinar a probabilidade de que a variável esteja dentro de um certo intervalo. Assim como ocorre nos modelos discretos, os modelos contínuos também possuem estruturas matemáticas bem definidas, consolidados na teoria das probabilidades. A seguir, apresentam-se os principais modelos contínuos.
...
```{r}
# Gera 1 milhao de numeros aleatorios de uma distribuicao uniforme continua
# no intervalo de 0 a 5
uniforme <- runif(1000000, min=0, max=5)

# Cria um data frame com os valores gerados
df_unif <- data.frame(uniforme)

# Cria o grafico da densidade dos valores gerados
...
```{r}
# Gera 100.000 numeros aleatorios de uma distribuicao exponencial
# com taxa (lambda) igual a 1
exponencial <- rexp(100000, 1)

# Cria um data frame com os valores gerados
df_exp <- data.frame(exponencial)

# Cria o grafico de densidade dos valores da distribuicao exponencial
...
```{r}
# Garante reprodutibilidade dos resultados (a mesma amostra sera gerada toda vez)
set.seed(2402)

# Gera 1 milhao de numeros aleatorios de uma distribuicao normal padrao (media = 0, desvio padrao = 1)
normal <- rnorm(1000000)

# Cria um data frame com a amostra gerada
df_normal <- data.frame(normal)

# Cria o grafico de densidade da amostra normal
...
Retomando os exemplos apresentados na introdução do capítulo, suponha, primeiramente, que o parâmetro de interesse seja a idade média das gestantes e puérperas hospitalizadas por COVID-19 que vieram a óbito em decorrência da doença no período de março de 2020 a dezembro de 2021, a qual denotaremos por $\mu$. Como temos acesso a todos os registros dessa população, o valor desse parâmetro não é desconhecido, mas isso servirá de auxílio para exemplificar os métodos que aqui serão empregados. Criando um vetor contendo todos os elementos da população, temos:

```{r}
# Le um arquivo RDS contendo os dados limpos sobre gestantes e puerperas com COVID-19
dados <- readr::read_rds("dados/dados_covid[LIMPO].rds")

# Cria um vetor contendo a idade, em anos, apenas das pacientes que foram a obito
populacao1 <- dados$idade_anos[which(dados$evolucao == "óbito")]

# Retorna o numero de elementos (ou seja, o tamanho da populacao de interesse)
length(populacao1)
```
...
#### Trabalhando com amostras de delineamento de amostragem aleatória simples

Para exemplificar os conceitos de estimação definidos anteriormente,  imularemos a retirada de amostras da população de gestantes e puérperas com a qual estamos trabalhando. Será aplicado um tipo de delineamento amostral chamado amostragem aleatória simples. Como já introduzido, esse delineamento é probabilistico e ocorre em dois tipos: amostragem aleatória simples com reposição e sem reposição. Um delineamento é definido como probabilistico se todas as unidades da população têm uma probabilidade conhecida e diferente de zero de serem selecionadas para a amostra. Definindo formalmente, considere uma população $U$ de tamanho $N$ e uma amostra de tamanho $n$, um delineamento é probabilistico quando cada elemento $i \in U$ tem probabilidade $P(i\in amostra) \neq 0$. Na amostragem aleatória simples com reposição, cada unidade da população pode ser selecionada mais de uma vez. Isso significa que, após a seleção de uma unidade, ela é devolvida à população antes da próxima seleção. Assim, a probabilidade de seleção de cada unidade permanece constante a cada sorteio, ou seja, $P(i \in amostra) = frac{n}{N}.
...
```{r}
# Define uma semente para garantir reprodutibilidade da amostragem
set.seed(43)

# Sorteia 30 elementos da populacao 'populacao1' com reposicao
...
```{r}
set.seed(312)
# Sorteia 50 elementos da populacao 'populacao2_transformada' com reposicao
...
M <- 100
n <- 15
# Gera M amostras de tamanho n da populacao 'populacao1', com reposicao
...
```{r}
# Calcula a media de cada amostra da população 1
x_barras <- apply(X = amostras_pop1, MARGIN = 2, FUN = mean)
head(x_barras)

# Calcula a media de cada amostra da população 2
...
Como o intuito deste livro não é conter uma porção pesada de teoria, introduziremos o conceito de intervalos de confiança a partir de exemplos, realizando explicações sobre os elementos envolvidos em sua construção conforme seja necessário. Caso seja de seu interesse, já publicamos, no site do Observatório, um texto que pode te ajudar a entender o melhor a teoria por trás dos intervalos de confiança, que conta também com o detalhamento de um dos principais métodos utilizados para a construção desses intervalos: o **método da quantidade pivotal**. O post está disponível <a href = https://observatorioobstetricobr.org/ciencia-de-dados/intervalos-de-confianca/ target = _blank> neste link </a>. Com isso em mente, prossigamos para nosso primeiro exemplo: a criação de intervalos de confiança para a média amostral.
...
## Cálculo do tamanho amostral para estimação precisa

Considerando os métodos de estimação que foram apresentados, ainda há uma questão importante que não foi tratada neste capítulo: como definir o tamanho de uma amostra. Não existe valor pré-estabelecido para tamanho de uma amostra de interesse, esse valor, em resumo, é definido para cada caso dependendo essencialmente de duas condições:objetivo do estudo e tipo de desfecho analisado.

É fundamental que a amostra seja suficientemente precisa para tornar possível a realização de inferências confiáveis. Uma amostra adequada deve ter poder estatístico suficiente para detectar diferenças ou efeitos presentes na população, bem como, precisão adequada, normalmente avaliada pela margem de erro cálculada pelo erro quadrático médio.

O cálculo do tamanho amostral leva em conta três componentes essenciais: (1) a variabilidade esperada do fenômeno estudado (σ² ou p(1-p)), (2) a margem de erro (E) para as estimativas, e (3) o nível de confiança desejado (1-α). Esses elementos se relacionam diretamente com as propriedades dos estimadores discutidas anteriormente, como não-vício e consistência, garantindo que as conclusões do estudo sejam tanto precisas quanto estatisticamente significativas.

Nesta seção, será apresentado o cálculo de tamanho amostral ideal considerando o delineamento de amostragem aleatória simples com reposição, já exemplicado ao longo desse capitulo.

### Cálculo de tamanho amostral para média com variância conhecida

O objetivo é estimar o *n* ideal de tal forma que o estimador considerado tenha um erro máximo de estimação igual a E, ou seja:

$$ P ( |\bar{Y} - \mu| \leq E) =  1 - \alpha$$
Retomando o intervalo de confiança para a média amostral quando a variância populacional é conhecida calculado, temos que

$$
IC(\mu,\ 1 - \alpha) = \left(\bar{X} - z_{1 - \alpha/2}\frac{\sigma}{\sqrt{n}};\; \bar{X} + z_{1 - \alpha/2}\frac{\sigma}{\sqrt{n}}\right).
$$
Sendo $E = z_{1 - \alpha/2}\frac{\sigma}{\sqrt{n}}$. Assim, isolando o n, temos:

$$n = \frac{ \sigma^2}{(E/z_{1-\alpha/2})^2}$$

Retomando o exemplo de retirada de amostras da população de gestantes e puérperas, obtivemos com n = 30 uma estimativa pontual para a idade média das gestantes e puérperas que faleceram em decorrência da COVID-19 é de $30.73$. Recalculando com 

```{r}
set.seed(533)

E <- 2        # Margem de erro de +- 2 anos
alpha <- 0.05 # Nível de significância (95% de confiança)

# Valor crítico z (usando qnorm)
z_critical <- qnorm(1 - alpha/2)  # 1.96 para 95% de confiança

# Fórmula do tamanho da amostra
n <- var(populacao1) / ((E / z_critical)^2)
...
OBSERVATÓRIO OBSTÉTRICO BRASILEIRO. *Função de verossimilhança*. Disponível em: https://observatorioobstetricobr.org/ciencia-de-dados/funcao-de-verossimilhanca/. Acesso em: 8 jun. 2025.
